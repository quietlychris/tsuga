...
#[test]
#[serial]
fn small_fully_connected_network_w_opencl() {
...
#[test]
#[serial]
fn big_fully_connected_network_w_opencl() {
    // Remember, the `n` value of both the input and output sets must be the same
    let n = 500;
    let input = Array::random((n, 10), Uniform::new(0., 1.));
    let output = Array::random((n, 2), Uniform::new(0., 1.));
