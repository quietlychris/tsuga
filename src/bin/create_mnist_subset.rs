...
fn main() {
    ...
    if Path::new(&CONSOLIDATED_TRAIN_DIR).exists() {
        fs::remove_dir_all(&CONSOLIDATED_TRAIN_DIR)
            .expect("Couldn't remove previous consolidated training directory");
    }
    std::thread::sleep(std::time::Duration::from_millis(10));
    fs::create_dir_all(&CONSOLIDATED_TRAIN_DIR)
        .expect("Couldn't build the consolidated training directory");
    ...
    if Path::new(&CONSOLIDATED_TEST_DIR).exists() {
        fs::remove_dir_all(&CONSOLIDATED_TEST_DIR)
            .expect("Couldn't remove previous consolidated training directory");
    }
    fs::create_dir_all(&CONSOLIDATED_TEST_DIR)
        .expect("Couldn't build the consolidated testing directory");
    ...
    for group in &groups {
        if group.len() >= TRAIN_SET_SIZE {
            for i in 0..TRAIN_SET_SIZE {
                ...
                fs::copy(
                    &group[i],
                    CONSOLIDATED_TRAIN_DIR.clone() + "/" + subpaths.last().unwrap(),
                )
                .unwrap(); // Copy foo.txt to bar.txt
            }
        }
    }
    ...
    for group in &groups {
        if group.len() >= TEST_SET_SIZE {
            for i in 0..TEST_SET_SIZE {
                ...
                fs::copy(
                    &group[i],
                    CONSOLIDATED_TEST_DIR.clone() + "/" + subpaths.last().unwrap(),
                )
                .unwrap(); // Copy foo.txt to bar.txt
            }
        }
